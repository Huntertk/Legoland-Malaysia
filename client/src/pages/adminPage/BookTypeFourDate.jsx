...
const BookTypeFourDate = () => {
    ...
    const disabledDays = blockedDates?.map((dates) => new Date(dates.blockDates))
      ...
      const addBlockDate = async () => {
        try {
          setIsLoading(true)
            const {data} = await axios.post('/api/v1/booktype-four-dates-manage/block-dates', {blockDates: dateToString})
        ...
        const removeBlockedDate = async (id) => {
          try {
            setIsLoading(true)
            const res = await axios.delete(`/api/v1/booktype-four-dates-manage/block-dates/${id}`)
            setIsFetch(prev => !prev)
            console.log(res);
            setIsLoading(false)
          } catch (error) {
  ...
  return (
    <div className='mainDateManageContainer'>
        <h1>1 Day Legoland Combo : Themepark + Sealife <br /> Date Manage</h1>
        <DayPicker
...
export default BookTypeFourDate